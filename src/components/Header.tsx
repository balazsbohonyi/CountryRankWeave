...
export default function Header({ dataset, topN, periodBId }: HeaderProps) {
  ...
  return (
    <header className="w-full max-w-4xl mx-auto text-center px-4 pt-10 pb-6 flex flex-col items-center">
      ...
      <span className="font-display tracking-[0.25em] text-[#cfff3b] font-black text-xs sm:text-sm uppercase mb-1 drop-shadow-sm select-none">
        {meta.datasetLabel ? meta.datasetLabel.toUpperCase() : "COMPARATIVE VISUALIZATION"}
      </span>

      {/* Massive Visual Capitalist Style Title */}
      <h1 className="font-serif text-4xl sm:text-6xl md:text-7xl font-black text-white italic tracking-tight leading-none drop-shadow-md select-none border-b border-[#222] pb-4 px-12">
        {meta.title}
      </h1>
